...
- Дублирующийся ключ `env:` в шаге `Build self-contained .deb + .AppImage`
  файла `release.yml` (1.6.4). YAML не разрешает повторных ключей в map,
  GitHub Actions отказывался запускать workflow и помечал push'и как failed
  без выполнения единого шага. Лишний блок удалён, оба env-ключа объединены.
  Python's `yaml.safe_load` молча перезаписывает дубликаты — добавил локально
  строгую проверку, которая эту ошибку отловила бы.
