...
**v1.3.0** с четырьмя STT-движками:
...
- [x] CUDA ExecutionProvider добавлен в GigaAM и Qwen3-ASR сессии
- [x] Автоматический fallback на CPU если GPU/CUDA недоступен
- [ ] Бенчмарки: CPU INT8 vs GPU FP32
- [ ] Настройка в UI: выбор GPU/CPU (Phase 8)
...
- [x] Интеграция Qwen3-ASR-0.6B через ONNX Runtime (4 сессии + BPE tokenizer)
- [x] Whisper-совместимая mel-спектрограмма (128 bins, Slaney)
- [x] Авторегрессивный decoder с KV-кешем
- [x] Скачивание множества файлов с прогресс-баром [N/M]
- [x] Умная проверка установки моделей (ключевые файлы, не только директория)

---

## Phase 8: UX-улучшения (оставшееся)

- [x] VAD — авто-стоп записи при тишине после речи (vad_enabled, vad_silence_secs)
- [x] Встроенное скачивание моделей с прогресс-баром (reqwest + Tauri events)
- [x] Автоочистка старых записей (настраиваемый период, периодическая)
- [x] Автозапуск при входе в систему (.desktop в autostart)
- [x] Автоскачивание GigaAM v3 при первом запуске
- [x] Авторегистрация хоткеев в GNOME при первом запуске (Wayland)
- [x] Раздел «О приложении» в меню (v1.0.0)
- [x] Версионирование v1.0.0 + CHANGELOG.md
- [ ] Streaming частичных результатов (см. исследование ниже)
- [ ] Выбор языка распознавания (для мультиязычных движков: Whisper, Qwen3-ASR)
- [ ] Автоопределение локали системы для языка распознавания по умолчанию
- [ ] Настройка параметров inference (количество потоков, GPU/CPU)
- [ ] i18n интерфейса: русский (по умолч.) + английский
...
- Каждые ~1.5 сек запускать inference на накопленном аудио
- Partial result отправлять на фронтенд через `app.emit("partial-result")`
- Финальный результат — как сейчас, по окончании записи
- Не требует новых зависимостей, использует загруженную модель
- GigaAM ~200-400мс на 5 сек аудио → приемлемо для pseudo-streaming
...
- [x] Автоматическое создание UDP-скриптов при первом запуске на Wayland
- [x] Установка скриптов в `~/.config/arcanaglyph/scripts/`
- [x] XDG-стандартные пути: модели, конфиг, кэш, скрипты
- [x] .deb пакет с зависимостями через Tauri
...
1. **Пунктуация** для Vosk, Whisper, Qwen3-ASR — Phase 9
2. **Выбор языка распознавания** + автоопределение локали — Phase 8
3. **i18n интерфейса** (русский + английский) — Phase 8
4. **Pseudo-streaming** с GigaAM CTC — Phase 8
5. **macOS** — Phase 10.2 (Tauri v2 уже поддерживает macOS)
6. **Flatpak / Snap** — Phase 10.4
